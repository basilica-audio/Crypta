...
namespace
{
    ...
    constexpr double biasThreshold = 0.1;
    constexpr double biasDepthVolts = 1.0;
        ...
        for (size_t sample = 0; sample < numSamples; ++sample)
        {
            ...
            switch (voicing)
            {
                ...
                case VoicingType::wool:
                {
                    ...
                    const auto driven = highDriveGain * x - biasDepthVolts * state.biasEnvelope + biasOffset;
                    shaped = state.shaper.process (driven, woolCurve);
                    break;
                }
